...
class ChooseDeliveryPackage(models.TransientModel):
        ...
        for rec in self:
            if rec.package_count < 1:
                raise ValidationError(
                    "Package count must be greater than or equal to 1."
                )
    ...
    @api.depends('delivery_package_type_id', 'package_count', 'picking_id', 'shipping_weight', 'package_shipping_weight')
    def _compute_package_help_text(self):
        for rec in self:
            help_text = _(
                "Making <b>%(package_count)s</b> package(s) using <b>%(package_type)s</b> " \
                "with a total shipping weight of <b>%(total_weight)s</b> <b>%(weight_uom_name)s</b>. " \
                "This will result in a shipping weight of "
                "<b>%(shipping_weight)s</b> <b>%(weight_uom_name)s</b> per package.") % {
