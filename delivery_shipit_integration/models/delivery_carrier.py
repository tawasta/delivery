...
class DeliveryCarrier(models.Model):
    ...
    def _shipit_get_additional_services(self, picking):
        ...
        for service in picking.shipit_additional_service_ids:
            if service.code in ["dng", "dangerousGoods", "lq", "limitedQuantities"]:
                raise ValidationError(
                    self.env._("Sending dangerous goods is not implemented")
                )
    ...
    def _shipit_validate_required_fields(self, picking, sender, receiver):
        ...
        if not self.shipit_service_code:
            missing_fields.append(self.env._("Carrier Shipit service ID"))

        required_sender_fields = {
            self.env._("Sender name"): sender.get("name"),
            self.env._("Sender street"): sender.get("address"),
            self.env._("Sender postal code"): sender.get("postcode"),
            self.env._("Sender city"): sender.get("city"),
            self.env._("Sender country code"): sender.get("country"),
            self.env._("Sender email"): sender.get("email"),
        }
        ...
        required_recipient_fields = {
            self.env._("Recipient name"): receiver.get("name"),
            self.env._("Recipient street"): receiver.get("address"),
            self.env._("Recipient postal code"): receiver.get("postcode"),
            self.env._("Recipient city"): receiver.get("city"),
            self.env._("Recipient country code"): receiver.get("country"),
            self.env._("Recipient email"): receiver.get("email"),
            self.env._("Recipient phone"): receiver.get("phone"),
        }
        ...
        if picking.shipping_weight <= 0 and picking.weight <= 0:
            missing_fields.append(self.env._("Shipment weight"))

        if self.shipit_require_pickup_point and not picking.shipit_pickup_point_id:
            missing_fields.append(self.env._("Pickup point ID"))
        ...
        if (
            ...
            missing_fields.append(
                self.env._("Pickup point service ID requires pickup point selection")
            )
    ...
    def rate_shipment(self, order):
        if self._shipit_is_carrier():
            raise UserError(
                self.env._("Shipit integration does not yet support 'Get rate'")
            )
        else:
